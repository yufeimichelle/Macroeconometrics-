...
[Figure 2: ACF plots] shows that for GDP and CPI have non-zero auto correlations for at least 10 lags, for the ERA, MB, Short_R have non-zero auto correlations for at least 2.5 lags.

[Figure 3: PACF plots] shows no statistically significant lags except short-term interest rate. Short-term interest rate has some spikes in lag 1 and 1.75 but not strong.
...
#| message: false
par(mfrow = c(3, 2))
pacf(exchange_rate_usd, lag.max = 10, plot = TRUE, main = "Exchange Rate (AUD/USD) ACF")
pacf(MB, lag.max = 10, plot = TRUE, main = "Monetary Base (M1) ACF")
pacf(Short_R, lag.max = 10, plot = TRUE , main = "Short-Term Interest Rate ACF")
pacf(GDP, lag.max = 10, plot = TRUE , main = "Gross Domestic Product ACF")
pacf(CPI, lag.max = 10, plot = TRUE , main = "Consumer Price Index ACF")
```
